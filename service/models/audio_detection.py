...
class AudioDetectionModule:
    def __init__(self,
        ...
        logger.info(f'历史缓冲区已设置，最多存储 {history_maxlen} 个前置缓冲区')
        ...
        if self.orig_sample_rate != self.target_sample_rate:
            ...
            self.resampler = None
            logger.info('采样率匹配，无需重采样器')

        logger.info('Silero VAD 初始化完成')
        logger.info(f'初始状态: {self.state.name}')
        ...
        try:
            ...
            while not self.audio_queue.empty():
                try:
                    ...
                    break
            logger.info(f"清空 audio_queue，共清除 {cleared} 个音频块。")
    ...
    async def periodic_logger(self):
        ...
        try:
            while True:
                logger.info("audio_detection run 函数正在正常运行。")
    ...
    async def run(self):
        ...
        while True:
            ...
            # logger.info(f'接收到音频数据，长度: {len(audio_data)}')
            ...
            # logger.info(f"追加到缓冲区，当前缓冲区长度: {len(self.buffer)}")

            # 当缓冲区足够大时，进行 VAD
            if len(self.buffer) >= self.buffer_size:
                # logger.info("缓冲区大小达到阈值，开始进行语音活动检测。")
                try:
                    # 从 buffer 中提取有效音频的时间段 [{'start': 100, 'end': 300}, {'start': 500, 'end': 700}]
                ...
                if speech_timestamps:
                    ...
                    if self.state == DetectionState.DURING_SPEECH:
                        ...
                        logger.info("检测到完整语音片段，已放入 detected_audio_queue。")
                        await self.history_audio_queue.put(audio_bytes)
                        logger.info("检测到完整语音片段，已放入 history_audio_queue。")
                ...
                logger.debug(f"更新历史缓冲区，目前存储 {len(self.history_buffers)} 个前置缓冲区。")
